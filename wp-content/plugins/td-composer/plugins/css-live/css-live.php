...
/*
 * Important!
 * Everything is in a function. This permits to run code as plugin OR/AND as composer extension (it can be plugin and extension in the same time)
 */
...
if ( ! function_exists( 'load_css_live' ) ) {
	function load_css_live() {
		...
		if ( is_plugin_active( 'td-composer/td-composer.php' ) ) {
			define('TD_LIVE_CSS_EXTENSION_URL', plugins_url( 'td-composer' ) . '/plugins/css-live' );
		}
		...
		global $load_as_plugin;
		global $load_as_extension;
		global $load_in_composer;
		global $load_in_composer_iframe;

		// Initialize globals
		$load_as_plugin = is_plugin_active( 'css-live/css-live.php' );
		$load_as_extension = is_plugin_active( 'td-composer/td-composer.php' );
...
//			}
		...
		} else if ( $load_as_plugin || $load_as_extension ) {
		...
		function tdc_on_live_css_inject_editor() {
			...
			<div id="tdw-css-writer" style="display: none" class="tdw-drag-dialog tdc-window-sidebar">
				...
				<?php
				if ( ( $load_as_plugin || $load_as_extension ) && ! $load_in_composer ) {
				?>
				...
				<footer>
					...
					?>
						...
						<div class="tdw-more-info-text">Write CSS OR LESS and hit save. CTRL + SPACE for auto-complete. Made by
							<a target="_blank" href="https://www.wpion.com/">wpion.com</a></div>
		 ...
		 */
		if ( $load_as_plugin || $load_as_extension ) {
		...
		if ( ( ( $load_as_plugin || $load_as_extension ) && ! $load_in_composer ) || $load_in_composer_iframe ) {
			...
			function td_live_css_load_plugin_css() {
			    if ( defined('TDC_USE_LESS') && TDC_USE_LESS === true ) { // we use defined because this can run without that constant when td-composer is inactive
                    wp_enqueue_style( 'td_live_css_main', TDC_URL . '/td_less_style.css.php?part=td_live_css_main', false, false );
                } else {
		...
		if ( $load_as_extension && $load_in_composer ) {
			add_action( 'admin_enqueue_scripts', 'td_live_css_load_admin_css' );
			function td_live_css_load_admin_css() {

                if ( defined('TDC_USE_LESS') && TDC_USE_LESS === true ) { // we use defined because this can run without that constant when td-composer is inactive
                    wp_enqueue_style( 'td_live_css_td_wp_admin', TDC_URL . '/td_less_style.css.php?part=td_wp_admin', false, false );
                } else {
                    //wp_enqueue_style( 'td_live_css_td_wp_admin', TD_LIVE_CSS_EXTENSION_URL . '/assets/css/td_wp_admin.css', false, false );
                    wp_enqueue_style( 'td_live_css_td_wp_admin', TDC_URL . '/assets/css/td_wp_admin.css', false, false );
                }
		...
		//if ( ( $load_as_plugin || $load_as_extension ) && ! $load_in_composer_iframe ) {
		if ( $load_as_plugin ) {
			...
			function td_live_css_load_plugin_js() {
			    ...
			    if ( defined( 'TDC_DEPLOY_MODE' ) && TDC_DEPLOY_MODE == 'deploy' ) {
			        wp_enqueue_script('js_files_for_wp_admin', TDC_URL . '/assets/js/js_files_for_plugin_live_css.min.js', array('jquery', 'underscore'), TDC_VERSION, true);
			    } else {
					...
					wp_enqueue_script('td_live_css_state', TD_LIVE_CSS_URL . '/assets/js/tdLiveCssState.js', array('jquery'), TD_LIVE_CSS_VERSION, true);
					wp_enqueue_script('td_live_css_main', TD_LIVE_CSS_URL . '/assets/js/tdLiveCssMain.js', array('td_live_css_ace_ext_language_tools', 'underscore'), TD_LIVE_CSS_VERSION, true);
					...
					wp_enqueue_script('td_live_css_inject', TD_LIVE_CSS_URL . '/assets/js/tdLiveCssInject.js', array('td_live_css_ace_ext_language_tools', 'td_live_css_less'), TD_LIVE_CSS_VERSION, true);
		...
		if ( $load_as_extension ) {
			...
			if ( $load_in_composer ) {
				...
				function td_live_css_load_admin_js() {
					...
					// load the js
				    if ( defined( 'TDC_DEPLOY_MODE' ) && TDC_DEPLOY_MODE == 'deploy' ) {
				        wp_enqueue_script('js_files_for_wp_admin', TDC_URL . '/assets/js/js_files_for_extension_live_css.min.js', array('jquery', 'underscore'), TDC_VERSION, true);
				    } else {
				        tdc_util::enqueue_js_files_array(tdc_config::$js_files_for_extension_live_css, array('jquery', 'underscore'), TD_LIVE_CSS_EXTENSION_URL );
				    }

//					wp_enqueue_script( 'td_live_css_ace', TD_LIVE_CSS_EXTENSION_URL . '/assets/external/ace/ace.js', array( 'jquery' ), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script( 'td_live_css_ace_ext_language_tools', TD_LIVE_CSS_EXTENSION_URL . '/assets/external/ace/ext-language_tools.js', array( 'td_live_css_ace' ), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script( 'td_live_css_state', TD_LIVE_CSS_EXTENSION_URL . '/assets/js/tdLiveCssState.js', array( 'jquery' ), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script( 'td_live_css_less', TD_LIVE_CSS_EXTENSION_URL . '/assets/external/less.min.js', array( 'td_live_css_ace_ext_language_tools' ), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script( 'td_live_css_inject', TD_LIVE_CSS_EXTENSION_URL . '/assets/js/tdLiveCssInject.js', array(
//						'td_live_css_ace_ext_language_tools',
//						'td_live_css_less'
//					), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script( 'td_live_css_main_tdc', TD_LIVE_CSS_EXTENSION_URL . '/assets/js/tdLiveCssMainTdc.js', array( 'jquery' ), TD_LIVE_CSS_VERSION, true );
				}

			} else {
				add_action( 'wp_enqueue_scripts', 'td_live_css_load_admin_js' );
				function td_live_css_load_admin_js() {

					// load the js
				    if ( defined( 'TDC_DEPLOY_MODE' ) && TDC_DEPLOY_MODE == 'deploy' ) {
				        wp_enqueue_script('js_files_for_wp_admin', TDC_URL . '/assets/js/js_files_for_live_css.min.js', array('jquery', 'underscore'), TDC_VERSION, true);
				    } else {
				        tdc_util::enqueue_js_files_array(tdc_config::$js_files_for_extension_live_css, array('jquery', 'underscore'), TD_LIVE_CSS_EXTENSION_URL);
				    }
...
//						'td_live_css_less'
//					), TD_LIVE_CSS_VERSION, true );
//					wp_enqueue_script('td_live_css_main', TD_LIVE_CSS_EXTENSION_URL . '/assets/js/tdLiveCssMain.js', array('td_live_css_ace_ext_language_tools', 'underscore'), TD_LIVE_CSS_VERSION, true);
				}
			}
		}
